...
public class Convertitore {
	...
	//		System.out.println("RESULT SET NEO4J=\n"+resultSet.toString());
	//
		...
		while (resultSet.next()) {
			...
			for (int i = 0; i < total_rows; i++) {
				if(resultSet.getObject(i + 1) == null){
					...
					String tabellePuntoAttributo = resultSet.getMetaData().getColumnLabel(i + 1).toLowerCase();
					String[] splittamentotPa = tabellePuntoAttributo.split("\\.");
					if (splittamentotPa.length == 1)
						obj.addProperty(resultSet.getMetaData().getColumnLabel(i + 1).toLowerCase(), resultSet.getObject(i + 1).toString());
					else
						obj.addProperty(resultSet.getMetaData().getColumnLabel(i + 1).toLowerCase().split("\\.")[1], resultSet.getObject(i + 1).toString());
				}
